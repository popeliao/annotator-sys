...
def record(ACT, p, q):
    ...
    if missingYetNotOrg:
        # Record this type of MIS that we are interested of
        fBeside.write("%s\nRSP==  %s\nKEY==  %s\n" %("MIS", "", strRecorder2[q]))  
...
def dealDocly(f1,f2):
    ...
    while c1:
        ...
        if c1 and c2:
            ...
            fBeside.write("---------------------------------------------------------------------------------------------------------------------\n")
            fBeside.write("Summary for Doc" + str(docNum) + "\n")
            fBeside.write("-----------------\n") 
