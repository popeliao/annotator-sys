...
# -*- coding: utf-8 -*-
"""
~~~ city_locater_in_header.py ~~~
A adjusted version of City_locater_in_ORG to find city mentions in 
those <cname> annotations that has no <NE:CITY> inside
...
it conducts post-process that locate all city name inside <cname> annotations, 
...
class City_locater_in_header(object):
    ...
    def process(self):
        ...
        p = re.compile(r'<cname>((?!NE:CITY).)*?</cname>')
        #the re would ensure that there is no CITY mentions found inside
        pmatch = re.compile(r'<cname>(.*?)</cname>')
...
def main(argv):
    ...
    fin = open(argv[1],"r")
    fout = open(argv[2],"w")        
    doer = City_locater_in_header(f, fin, fout)
    doer.readIn(None)
    doer.process()    
    #Test cases
